...
internal class LevenshteinComparer : IEqualityComparer<string>
{
    ...
    {
        var distance = _Levenshtein.iLD(x, y);
...
public class Levenshtein
{
    ...
    {
        int[,] matrix; // matrix
        var sNewLen = sNew.Length; // length of sNew
        var sOldLen = sOld.Length; // length of sOld
        ...
        if (Math.Max(sNew.Length, sOld.Length) > Math.Pow(2, 31))
            throw new Exception("\nMaximum string length in Levenshtein.LD is " + Math.Pow(2, 31) + ".\nYours is " +
                                Math.Max(sNew.Length, sOld.Length) + ".");

        // Step 1

        if (sNewLen == 0) return sOldLen;

        if (sOldLen == 0) return sNewLen;

        matrix = new int[sNewLen + 1, sOldLen + 1];

        // Step 2

        for (sNewIdx = 0; sNewIdx <= sNewLen; sNewIdx++) matrix[sNewIdx, 0] = sNewIdx;

        for (sOldIdx = 0; sOldIdx <= sOldLen; sOldIdx++) matrix[0, sOldIdx] = sOldIdx;
        ...
        var max = Math.Max(sNewLen, sOldLen);
