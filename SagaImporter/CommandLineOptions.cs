...
internal class OutputOptions
{
    ...
    [Option('a', "WriteAuthors", Required = false,
        HelpText =
            "Write out a set of directories for authors with their books underneath, defaults to Authors/{Author Name}/{Book Name}")]
    public bool WriteAuthors { get; set; }

    [Option('s', "WriteSeries", Required = false,
        HelpText =
            "Write out a set of directories for series with their books underneath, defaults to Series/{Series Name}/{Book Name}")]
    public bool WriteSeries { get; set; }

    [Option('g', "WriteGenre", Required = false,
        HelpText =
            "Write out a set of directories for genres with their books underneath, defaults to Genre/{Genre Name}/{Book Name}")]
    public bool WriteGenre { get; set; }

    [Option('r', "WriteReports", Required = false,
        HelpText = "Write out a reports directory that contains the hint csv for editing")]
    public bool WriteReports { get; set; }

    [Option('t', "TopLevelOverride", Required = false,
        HelpText = "Override the toplevel directory name when writing out from Authors/Series/Genres respectively. ")]
    public string TopLevelOverride { get; set; }

    [Option('e', "WriteDescriptions", Required = false,
        HelpText = "Write out desc.txt files for all looked up files. ")]
    public bool WriteDescriptions { get; set; }

    [Option('b', "WriteBooks", Required = false,
        HelpText =
            "Write out all the books to a books folder. Not really useful except for fidning for fixing things ")]
    public bool WriteBooks { get; set; }

    [Option('f', "WriteFailBooks", Required = false,
        HelpText =
            "Write out all the books to a books folder. Not really useful except for fidning for fixing things ")]
...
internal class LookupOptions
{
    ...
    [Option('f', "hintFile", Required = false,
        HelpText = "Search for missing goodreads lookups using hint file pointing to book pages")]
    ...
    [Option('s', "Series", Required = false,
        HelpText = "Do good reads lookups of series entries, backfill series entries as needed")]
    public bool Series { get; set; }

    [Option('a', "Authors", Required = false, HelpText = "Do good reads lookups of author entries")]
    public bool Authors { get; set; }

    [Option('p', "PurgeAndRebuild", Required = false,
        HelpText = "This will purge the series and author tables and rebuild them by using the good reads data")]
    public bool PurgeAndRebuild { get; set; }

    [Option('i', "Images", Required = false,
        HelpText = "Look up good reads author and cover images and add them to the database")]
...
internal class DumpOptions
{
    ...
    [Option('f', "FailedLookups", Required = false,
        HelpText = "Dump out all the books that failed a good reads lookup")]
