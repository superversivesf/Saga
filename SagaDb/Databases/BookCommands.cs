...
public class BookCommands
{
    ...
    public BookCommands()
    {
        ...
        _dbFile = @"audiobook-test.db";
        using var db = new BookOrganizerContext(_dbFile);
    ...
    public BookOrganizerContext InitContext(BookOrganizerContext context)
    {
        if (context == null)
        {
            context = new BookOrganizerContext(_dbFile);
    ...
    {
        db = InitContext(db);
        var _entry = db.BookSeries.FirstOrDefault(a =>
            a.BookId == _book.BookId && a.SeriesId == _series.SeriesId && a.SeriesVolume == _bookVolume);
    ...
    public List<BookSerie> GetSeriesBooks(string seriesId, BookOrganizerContext db = null)
    {
        ...
        var _seriesBooks = db.BookSeries.Where(bs => bs.SeriesId == seriesId).ToList()
            .OrderBy(bs => ConvertSeriesVolumeToDouble(bs.SeriesVolume)).ToList();
    ...
    public List<AudioFile> GetAudioFilesByBookId(string bookId, BookOrganizerContext db = null)
    {
        ...
        var _audioFileIdList = db.AudioFiles.Where(f => _bookFiles.Contains(f.AudioFileId))
            .OrderBy(af => af.AudioFileName).ToList();
    ...
    public List<SeriesInfo> GetBookSeriesByBookId(string bookId, BookOrganizerContext db = null)
    {
        ...
        foreach (var bs in _bookSeries)
        {
            ...
            _result.Add(new SeriesInfo
            {
                SeriesName = _series.SeriesName, SeriesVolume = bs.SeriesVolume, SeriesId = _series.SeriesId,
                SeriesLink = _series.SeriesLink
            });
        }
    ...
    public void LinkAuthorToBook(List<Author> authorEntries, Book _book, AuthorType _type,
        BookOrganizerContext db = null)
    {
        db = InitContext(db);
    ...
    public void DumpBooks(BookOrganizerContext db = null)
    {
        ...
        foreach (var a in db.Books) Console.WriteLine(a.BookId + " -> " + a.BookTitle);
    }
    ...
    public List<Book> GetBooksMissingLookupAndFailedLookup(BookOrganizerContext db = null)
    {
        db = InitContext(db);

        return db.Books.Where(b => b.LookupFetchTried == false).OrderBy(b => b.BookTitle).ToList();
    }

    public List<Book> GetBooksMissingLookup(BookOrganizerContext db = null)
    {
        db = InitContext(db);
    ...
    public List<Book> GetBooksFailedLookup(BookOrganizerContext db = null)
    {
        db = InitContext(db);

        return db.Books.Where(b => b.LookupFetchTried == true && string.IsNullOrEmpty(b.LookupDescription))
            .OrderBy(b => b.BookTitle).ToList();
    }
    ...
    public void DumpAuthors(BookOrganizerContext db = null)
    {
        ...
        foreach (var a in db.Authors) Console.WriteLine(a.AuthorId + " -> " + a.AuthorName);
    }
    ...
    {
        db = InitContext(db);
        return db.Books.Where(b => b.LookupLink == link).ToList();
    }
    ...
    public void DumpAudiofiles(BookOrganizerContext db = null)
    {
        ...
        foreach (var a in db.AudioFiles) Console.WriteLine(a.AudioFileId + " -> " + a.AudioFileName);
    }
    ...
    public void DumpGenres(BookOrganizerContext db = null)
    {
        ...
        foreach (var a in db.Genres) Console.WriteLine(a.GenreId + " -> " + a.GenreName);
    }
    ...
    public void DumpSeries(BookOrganizerContext db = null)
    {
        ...
        foreach (var a in db.Series) Console.WriteLine(a.SeriesId + " -> " + a.SeriesName);
    }
